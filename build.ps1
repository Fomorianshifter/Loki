...
function Convert-ToWslPath {
    ...
    return [regex]::Replace($normalized, '^([A-Za-z]):', {
        param($match)
        "/mnt/$($match.Groups[1].Value.ToLower())"
    })
}

function Get-WslInvocationArguments {
    $distros = @()
...
if (-not $compiler) {
    ...
    if ($wsl) {
        ...
        $workspaceLinuxPath = Convert-ToWslPath -WindowsPath $workspacePath
        $wslCommand = "set -e; cd '$workspaceLinuxPath'; chmod +x ./build_native.sh; ./build_native.sh $Mode"
        $wslArgs = Get-WslInvocationArguments

        Write-Host "[!] arm-linux-gnueabihf-gcc not found on Windows" -ForegroundColor Yellow
        Write-Host "[>] Falling back to WSL using build_native.sh..." -ForegroundColor Cyan
        & wsl.exe @wslArgs bash -lc $wslCommand

        if ($LASTEXITCODE -ne 0) {
            Write-Host "[X] WSL build failed" -ForegroundColor Red
            Write-Host "[!] In WSL Ubuntu, run: sudo apt-get update && sudo apt-get install gcc-arm-linux-gnueabihf" -ForegroundColor Yellow
            Write-Host "[!] Native Windows fallback: https://releases.linaro.org/components/toolchain/binaries/latest-7/arm-linux-gnueabihf/" -ForegroundColor Yellow
            exit 1
        }

        $buildViaWsl = $true
        Write-Host "[+] WSL build complete" -ForegroundColor Green
    } else {
        Write-Host "[X] arm-linux-gnueabihf-gcc not found!" -ForegroundColor Red
        Write-Host "[!] Windows (recommended): install WSL Ubuntu and run: sudo apt-get install gcc-arm-linux-gnueabihf" -ForegroundColor Yellow
        Write-Host "[!] Native Windows fallback: https://releases.linaro.org/components/toolchain/binaries/latest-7/arm-linux-gnueabihf/" -ForegroundColor Yellow
        exit 1
    }
} else {
